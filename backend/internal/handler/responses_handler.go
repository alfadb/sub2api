...
func NewResponsesHandler(
	...
	var responseHeaderFilter *responseheaders.CompiledHeaderFilter
	if cfg != nil {
		responseHeaderFilter = responseheaders.CompileHeaderFilter(cfg.Security.ResponseHeaders)
	}
...
func (h *ResponsesHandler) forwardResponses(c *gin.Context, account *service.Account, body []byte, isStream bool, start time.Time) (*service.ForwardResult, error) {
	if !isStream {
...
func (h *ResponsesHandler) processResponsesStream(c *gin.Context, resp *http.Response, model string, start time.Time) (*service.ForwardResult, error) {
	...
	// Build usage result
	var resultUsage service.ClaudeUsage
	if usage != nil {
		resultUsage = service.ClaudeUsage{
	...
	logger.L().Debug("responses.stream_completed",
		zap.String("request_id", requestID),
		zap.Int("input_tokens", resultUsage.InputTokens),
		zap.Int("output_tokens", resultUsage.OutputTokens),
	)

...
// usageExtractor extracts ClaudeUsage from a gjson usage object.
type usageExtractor func(u gjson.Result) ClaudeUsage
...
func (s *CopilotGatewayService) Forward(ctx context.Context, c *gin.Context, account *Account, body []byte) (*ForwardResult, error) {
...
func (s *CopilotGatewayService) ForwardResponses(ctx context.Context, c *gin.Context, account *Account, body []byte) (*ForwardResult, error) {
...
type convertedResponseHandler struct {
	handleStream    func(c *gin.Context, resp *http.Response, model string, start time.Time) (*ForwardResult, error)
	handleNonStream func(c *gin.Context, resp *http.Response, model string, start time.Time) (*ForwardResult, error)
}
...
func (s *CopilotGatewayService) ForwardChatAsAnthropic(ctx context.Context, c *gin.Context, account *Account, body []byte) (*ForwardResult, error) {
...
func (s *CopilotGatewayService) ForwardMessages(ctx context.Context, c *gin.Context, account *Account, body []byte) (*ForwardResult, error) {
	start := time.Now()
...
func (s *CopilotGatewayService) handleMessagesNonStreamResponse(c *gin.Context, resp *http.Response, model string, start time.Time) (*ForwardResult, error) {
	...
	logger.L().Debug("copilot messages non-stream: usage extracted",
		...
		zap.Int("completion_tokens", usage.OutputTokens),
...
func (s *CopilotGatewayService) handleMessagesStreamResponse(c *gin.Context, resp *http.Response, model string, start time.Time) (*ForwardResult, error) {
	requestID := resp.Header.Get("x-request-id")
...
func extractAnthropicUsage(body []byte) ClaudeUsage {
...
func (s *CopilotGatewayService) ForwardChatAsResponses(ctx context.Context, c *gin.Context, account *Account, body []byte) (*ForwardResult, error) {
...
func (s *CopilotGatewayService) handleResponsesToChatNonStream(c *gin.Context, resp *http.Response, model string, start time.Time) (*ForwardResult, error) {
	requestID := resp.Header.Get("x-request-id")
	...
	chatResp := apicompat.ResponsesToChatCompletions(&responsesResp, model)
...
func (s *CopilotGatewayService) handleResponsesToChatStream(c *gin.Context, resp *http.Response, model string, start time.Time) (*ForwardResult, error) {
	requestID := resp.Header.Get("x-request-id")
	...
	if !completionEventReceived {
		...
		if isCodexModel {
			...
			logger.L().Info("copilot responses-to-chat stream: ended without completion event (expected for Codex models)",
				...
				zap.Int("final_prompt_tokens", usage.InputTokens),
				zap.Int("final_completion_tokens", usage.OutputTokens),
...
func (s *CopilotGatewayService) handleAnthropicNonStreamResponse(c *gin.Context, resp *http.Response, model string, start time.Time) (*ForwardResult, error) {
	requestID := resp.Header.Get("x-request-id")
...
func (s *CopilotGatewayService) handleAnthropicStreamResponse(c *gin.Context, resp *http.Response, model string, start time.Time) (*ForwardResult, error) {
	requestID := resp.Header.Get("x-request-id")
	...
	logger.L().Debug("copilot anthropic stream: final usage",
		...
		zap.Int("completion_tokens", usage.OutputTokens),
		zap.Int("total_tokens", usage.InputTokens+usage.OutputTokens),
		zap.Bool("is_zero_usage", usage.InputTokens == 0 && usage.OutputTokens == 0),
	)
...
func (s *CopilotGatewayService) forward(ctx context.Context, c *gin.Context, account *Account, body []byte, cfg copilotForwardConfig) (*ForwardResult, error) {
	start := time.Now()
	...
	// Forward Anthropic-Beta header for thinking/reasoning model support.
	// opencode sets this for Claude models on /chat/completions; without it
	// Copilot silently disables extended thinking and falls back to normal mode.
	if beta := c.Request.Header.Get("Anthropic-Beta"); beta != "" {
		req.Header.Set("Anthropic-Beta", filterAnthropicBeta(beta))
	}
...
func (s *CopilotGatewayService) HandleResponsesError(c *gin.Context, resp *http.Response, account *Account, model string, stream bool, start time.Time) (*ForwardResult, error) {
...
func (s *CopilotGatewayService) handleErrorResponse(c *gin.Context, resp *http.Response, account *Account, model string, stream bool, start time.Time) (*ForwardResult, error) {
	respBody, err := io.ReadAll(resp.Body)
	if err != nil {
	...
	// Mark account status (rate-limited, temp-unschedulable, error, etc.)
	// so the scheduler skips it until recovery, matching other platform gateways.
	s.rateLimitService.HandleUpstreamError(c.Request.Context(), account, resp.StatusCode, resp.Header, respBody)
...
func (s *CopilotGatewayService) handleStreamResponse(c *gin.Context, resp *http.Response, model string, start time.Time, extract usageExtractor) (*ForwardResult, error) {
	requestID := resp.Header.Get("x-request-id")
		...
		if strings.HasPrefix(line, "data: ") && line != "data: [DONE]" {
			...
			if u.Exists() {
				...
				if u.Exists() {
					...
					usage = ClaudeUsage{InputTokens: pt, OutputTokens: ct}
				}
			}
		}
...
func (s *CopilotGatewayService) handleNonStreamResponse(c *gin.Context, resp *http.Response, model string, start time.Time, extract usageExtractor) (*ForwardResult, error) {
	requestID := resp.Header.Get("x-request-id")

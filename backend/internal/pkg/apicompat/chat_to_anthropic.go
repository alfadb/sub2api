...
func buildAnthropicContent(msg ChatMessage) []AnthropicContentBlock {
	...
	text, _ := parseChatContent(msg.Content)
	if text != "" {
...
func handleChatContent(
	...
	if (delta.Content == nil || *delta.Content == "") && delta.ReasoningOpaque != "" && state.ThinkingBlockOpen {
		idx := state.ContentBlockIndex
		*events = append(*events, AnthropicStreamEvent{
			Type:  "content_block_delta",
			Index: &idx,
...
func handleChatToolCalls(
	...
	for _, tc := range delta.ToolCalls {
		...
		if tc.ID != "" && tc.Function.Name != "" {
			...
			state.ToolCalls[derefInt(tc.Index)] = trackedToolCall{
				ID:                tc.ID,
				Name:              tc.Function.Name,
		...
		if tc.Function.Arguments != "" {
			if info, ok := state.ToolCalls[derefInt(tc.Index)]; ok {
...
func handleStreamReasoningOpaque(
	...
	if (delta.Content != nil && *delta.Content != "") || delta.ReasoningText != "" {
		return
	}
	idx := state.ContentBlockIndex
	*events = append(*events,

...
type copilotForwardFunc func(ctx context.Context, c *gin.Context, account *service.Account, body []byte) (*service.ForwardResult, error)
...
func (h *CopilotGatewayHandler) ChatCompletions(c *gin.Context) {
	h.handleForward(c, "chat_completions", func(ctx context.Context, c *gin.Context, account *service.Account, body []byte) (*service.ForwardResult, error) {
...
func (h *CopilotGatewayHandler) Messages(c *gin.Context) {
	h.handleForward(c, "messages", func(ctx context.Context, c *gin.Context, account *service.Account, body []byte) (*service.ForwardResult, error) {
...
func (h *CopilotGatewayHandler) handleForward(c *gin.Context, endpoint string, forwardFn copilotForwardFunc) {
	...
	for {
		...
		if !account.IsModelSupported(reqModel) {
			reqLog.Info("copilot.model_not_supported",
				zap.String("model", reqModel),
				zap.Int64("account_id", account.ID),
			)
			h.handleStreamingAwareError(c, http.StatusBadRequest, "invalid_request_error",
				fmt.Sprintf("Model '%s' is not supported by this account", reqModel), streamStarted)
			return
		}

		// 5. Acquire account concurrency slot

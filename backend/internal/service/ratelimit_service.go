...
func (s *RateLimitService) handle403(ctx context.Context, account *Account, upstreamMsg string, responseBody []byte) (shouldDisable bool) {
	...
	// 国产供应商、openai 与 ollama_cloud 同口径:HTML 403(CDN/代理拦截页)不构成账号
	// 失效证据,且 403 在 failover 状态集里会被逐账号重放——直接 SetError 会让一个
	// 坏请求/一层坏代理连环永久禁用整组账号。走 HTML 豁免 + N 次累计 + 临时冷却。
	if account.Platform == PlatformOpenAI || IsCNProvider(account.Platform) || account.IsOpenCodeGo() || account.IsOllamaCloud() {
